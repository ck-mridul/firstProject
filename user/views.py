...
def cart_view(request):
    ...
    if cart.coupon:
        ...
        coupon_price = 0
    discount = round(
        sum(item.product.get_offer_price(item.variant) * item.quantity for item in cart_items), 2
    )
    total -= discount
    context = {
...
def update_cart(request):
    ...
    if request.method == "POST":
        ...
        data = {"item_total": item_total, "sub_total": total_price, "total": total, "discount":discount}
