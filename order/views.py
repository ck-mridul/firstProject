...
def payment(request):
    ...
    if request.user.is_authenticated:
        order = Order.objects.get(
            user=request.user, is_orderd=False, order_number=OrdID
        )
        payment = Payment(
            user=request.user,
        ...
        order = Order.objects.get(
            session_id=session_key(request), is_orderd=False, order_number=OrdID
        )
        payment = Payment(
...
def place_order(request):
    ...
    if request.method == "POST":
        ...
        payment = client.order.create(
            {"amount": int(total), "currency": "INR", "payment_capture": 1}
        )
